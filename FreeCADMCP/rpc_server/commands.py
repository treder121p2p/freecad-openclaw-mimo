...
def register_commands() -> None:
    # In console (--console) mode, FreeCADGui exists but lacks addCommand.
    # GUI toolbar commands are not needed for RPC; skip gracefully.
    if not hasattr(FreeCADGui, "addCommand"):
        FreeCAD.Console.PrintMessage(
            "[MCP] Console mode: GUI commands skipped (RPC server still works).\n"
        )
        return
